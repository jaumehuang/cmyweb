...
//			$(".swiper-wrapper").height(Height);
			...
			$("body").on("click", "button", function() {
				...
				if($self.attr("data-returnMoney")) {

					window.location.href = "returnMoney.html?str=" + $self.attr("data-returnMoney");
				};
				...
				success: function(ret) {
					...
					if(!ret.isError) {
						...
						for(var i = 0; i < arryList.length; i++) {
							...
							str += '<li class="product_li" data-guid="' + arryList[i].OrderNumber + '">';
							...
							for(var j = 0; j < arryList2.length; j++) {
								...
								str += '<div class="fl"><span>' + arryList2[j].Attributes + '</span></div>';
								str += '<div class="fr">X' + arryList2[j].buyNumber + '</div></div>';

								str += '<div class="price">';
								str += '<div class="red_price"><span>' + arryList2[j].MarketPrice + '漫豆</span></div>';
							...
							if(arryList[i].PaymentStatus == 0) {

								str += '<span class="fl">待支付</span>';
								str += '<div class="btn_group fr">';
								str += '<button class="btn_cancel"  data-OrderNumber="' + arryList[i].OrderNumber + '" data-type="0">取消订单</button><button class="btn_pay" type="button" onclick="window.location.href=\'' + 'pay.html?str=' + arryList[i].OrderNumber + '\'">去支付</button>';

							}

							if(arryList[i].PaymentStatus == 2) {
								if(arryList[i].isBack == 0) {

									if(arryList[i].ShipmentStatus == 1) {
										var strOrder = arryList[i].Guid + "&" + arryList[i].OrderNumber;
										str += '<span class="fl">已发货</span>';
										str += '<div class="btn_group fr">';
										str += '<button class="btn_comment" data-returnMoney="' + strOrder + '"  >申请退款</button>';
										str += '<button class="btn_pay" data-OrderNumber="' + arryList[i].OrderNumber + '" data-type="1">确认收货</button>';
							...
							if(arryList[i].OderStatus == 5) {
								...
								str += '<span class="fl" style="margin-top:0.125rem">已支付</span>';
								str += '<div class="btn_group fr" style="margin-top:0;">';
